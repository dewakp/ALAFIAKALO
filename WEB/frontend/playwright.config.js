...
export default defineConfig({
  ...
  // Serve a real build rather than the dev server — locally AND in CI.
  //
  // Skipped only when PLAYWRIGHT_BASE_URL is set: that means the suite is
  // pointed at a server someone else is running — the `frontend-preview`
  // container, or a deployed URL — and starting a second one here would test the
  // wrong thing while fighting for the port.
  //
  // It used to be skipped in CI as well, where nothing else starts a server, so
  // every Web CI run failed every spec with ERR_CONNECTION_REFUSED.
  //
